...
router.route("/").get(async (req, res) => {
	...
	try {
		const result = await axios({
			method: "GET",
			url: `https://icademymiddleeast.instructure.com/api/v1/courses/${courseId}/modules`,
			headers: {
				...
				if (unit.items_count > 100) {
					...
					async function apiPagination() {
						...
						const itemRes = await axios({
							method: "GET",
							url: `https://icademymiddleeast.instructure.com/api/v1/courses/${courseId}/modules/${unit.id}/items`,
							headers: {
								Accept: "application/json+canvas-string-ids"
							},
							params: {
